...
def predictions(weather_turnstile):
    # weather_turnstile['week_day'] = weather_turnstile['DATEn'].map(lambda x: datetime.datetime.strptime(x,'%Y-%m-%d').day)
    ...
    #model = sm.OLS.from_formula(formula="ENTRIESn_hourly ~ rain + Hour + meantempi + 1", data=features)
...
def main():
    print "start"
    df = pandas.read_csv("improved-dataset/turnstile_data_master_with_weather.csv")
    predictions(df)
    print "done"
